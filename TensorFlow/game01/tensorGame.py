...
class Game(object):
    def __init__(self):
        ...
        self.ball_pos = pygame.Rect(self.ball_pos_x, self.ball_pos_y,
                                    BALL_SIZE[0], BALL_SIZE[1])

        self.bar_pos_x = SCREEN_SIZE[0] // 2 - BAR_SIZE[0] // 2
        self.bar_pos = pygame.Rect(self.bar_pos_x,
                                   SCREEN_SIZE[1] - BAR_SIZE[1], BAR_SIZE[0],
                                   BAR_SIZE[1])
    ...
    def step(self, action):
        ...
        if self.ball_pos.top <= 0 or self.ball_pos.bottom >= (
                SCREEN_SIZE[1] - BAR_SIZE[1] + 1):
        ...
        if self.bar_pos.top <= self.ball_pos.bottom and (
                self.bar_pos.left < self.ball_pos.right
...
def convolutional_neural_network(input_image):
    ...
    conv1 = tf.nn.relu(
        tf.nn.conv2d(
            ...
            weights['w_conv1'],
            strides=[1, 4, 4, 1],
            padding="VALID") + biases['b_conv1'])
    conv2 = tf.nn.relu(
        tf.nn.conv2d(
            conv1, weights['w_conv2'], strides=[1, 2, 2, 1], padding="VALID") +
        biases['b_conv2'])
    conv3 = tf.nn.relu(
        tf.nn.conv2d(
            conv2, weights['w_conv3'], strides=[1, 1, 1, 1], padding="VALID") +
        biases['b_conv3'])
...
def train_neural_network(input_image):
    ...
    action = tf.reduce_sum(
        tf.multiply(predict_action, argmax), reduction_indices=1)
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            image = cv2.cvtColor(
                cv2.resize(image, (100, 80)), cv2.COLOR_BGR2GRAY)
            ret, image = cv2.threshold(image, 1, 255, cv2.THRESH_BINARY)
            ...
            if n > OBSERVE:
                ...
                optimizer.run(
                    feed_dict={
                        gt: gt_batch,
                        argmax: argmax_batch,
                        input_image: input_image_data_batch
                    })
            ...
            print(n, "epsilon:", epsilon, " ", "action:", maxIndex, " ",
                  "reward:", reward)
